...
if ($v_acao == "LISTAR_DEPARTAMENTOS") {

    if (strpos($_SESSION["vs_array_access"], "T0022") > 0){

        $v_pos = strpos($_SESSION["vs_array_access"], "T0022");
        $v_cad_dep_ler = substr($_SESSION["vs_array_access"],$v_pos+8,1);
        $v_cad_dep_criar = substr($_SESSION["vs_array_access"],$v_pos+12,1);
        $v_cad_dep_gravar = substr($_SESSION["vs_array_access"],$v_pos+16,1);
        $v_cad_dep_excluir = substr($_SESSION["vs_array_access"],$v_pos+20,1);
    ...
    $v_sql = "SELECT count(id) as linhas from db_emp_".$_SESSION["vs_db_empresa"].".t_rh_departamentos " . $v_filtro;
    ...
    // GERANDO A LISTA
    $v_sql = "SELECT Id, Nome Departamento " .
        "FROM db_emp_".$_SESSION["vs_db_empresa"].".t_rh_departamentos " . $v_filtro . " ORDER BY " . $v_tab_campo . " " . $v_tab_ordem . " OFFSET " . $v_tab_sql_limit_in . " LIMIT " . $v_limit;
    ...
    while ($row = pg_fetch_assoc($result)) {
        $v_dados[] = array("Id" => $row["id"], "Departamento" => $row["departamento"]);
    }
...
if ($v_acao == "EV_SELECT") {
    ...
    $v_sql = "SELECT Id, Nome Departamento, Descricao " .
        "FROM db_emp_".$_SESSION["vs_db_empresa"].".t_rh_departamentos WHERE Id = ".$_POST["v_id"];
    $result = pg_query($conn, $v_sql);
...
if ($v_acao == "EV_NOVO") {
    ...
    $v_departamento = strtoupper(addslashes($_POST["v_departamento"]));
    $v_descricao = addslashes($_POST["v_descricao"]);
    

    $v_sql = "INSERT INTO t_rh_departamentos (Nome, Descricao)" . "\n" .
        "VALUES('" . $v_departamento . "','" . $v_descricao . "')";
...
if ($v_acao == "EV_SALVAR") {
    ...
    $v_sql = "UPDATE t_rh_departamentos SET \n" .
        "Nome = '" . $v_departamento . "', \n" .
...
if ($v_acao == "EV_EXCLUIR") {
    ...
    $v_sql = "DELETE FROM t_rh_departamentos WHERE Id = " . $v_id;
