...
if ($v_acao == "EV_LISTAR") {
    ...
    $v_sql = "SELECT count(empresa_portal) as linhas from db_adm_rh.t_empresas_integracao";
    ...
    while ($row = pg_fetch_assoc($result)) {
        $v_dados[] = array("empresa_portal" => $row["empresa_portal"], "empresa_erp" => $row["empresa_erp"], "nome" => $row["nome"]);
    }
...
if ($v_acao == "EV_SELECT") { //OK
    ...
    while ($row = pg_fetch_assoc($result)) {
        $v_dados[] = array(
            "empresa_portal" => $row["empresa_portal"],
...
if ($v_acao == "EV_NOVO") {
    ...
    $v_empresa_portal = addslashes($_POST["v_empresa_portal"]);
    $v_empresa_erp = addslashes($_POST["v_empresa_erp"]);



    $v_sql = "INSERT INTO db_adm_rh.t_empresas_integracao (empresa_portal, empresa_erp)" . "\n" .
        "VALUES('" . $v_empresa_portal . "','" . $v_empresa_erp . "')";
...
// SALVANDO REGISTRO
if ($v_acao == "EV_SALVAR") { //FAZER ESTE AGORA
    // var_dump("EV_SALVAR");
    // die;
    ...
    // $v_sql = "UPDATE db_adm_rh.t_empresas_integracao SET \n" .
    //     "empresa_portal = '" . $v_empresa_portal . "', \n" .
    //     "empresa_erp = " . $v_empresa_erp . ", \n";
    // var_dump($v_sql);
    // die;
    ...
    // $v_sql = "UPDATE db_adm_rh.t_empresas_integracao
    //             SET empresa_erp=36                   
    //                     where empresa_portal = 133";

    $v_sql = "UPDATE db_adm_rh.t_empresas_integracao
                SET empresa_erp = {$v_empresa_erp}                   
                    where empresa_portal = {$v_empresa_portal}";
...
if ($v_acao == "EV_EXCLUIR") {
    ...
    $v_sql = "DELETE FROM db_adm_rh.t_empresas_integracao
                WHERE empresa_portal = $v_empresa_portal";
