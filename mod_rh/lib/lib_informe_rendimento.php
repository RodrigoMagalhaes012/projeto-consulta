...
$vs_id_emp = addslashes($_SESSION["vs_id_empresa"]);
...
if ($v_acao == 'LISTAR_DOCUMENTOS') {
    ...
    $v_sql = "SELECT trp.nome_documento , trp.descricao, trp.ip_autenticacao, trp.usuario_autenticacao, trp.data_autenticacao, trp.ano_referencia, trp.id, trp.cpf, te.cnpj , te.id id_emp, trp.url_arquivo
...
if ($v_acao == "EV_AUTENTICA") {
    ...
    $v_sql = "UPDATE db_adm_rh.t_rh_informe_rendimento
			   ...
			   where id_empresa = {$_SESSION["vs_id_empresa"]} 
				 and cpf = '{$v_cpf}' 
                 and id = '{$v_id_informe}'
				 RETURNING data_autenticacao, ip_autenticacao, usuario_autenticacao, ano_referencia";
    ...
    $dados = array(
        ...
        "ano_referencia" => $v_autenticacao[3]
