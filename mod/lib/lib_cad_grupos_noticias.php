...
if ($v_acao == "LISTAR_GRUPOS") {
    ...
    // GERANDO A LISTA
    $v_sql = "SELECT * FROM db_adm.t_grupo_noticias";
...
if ($v_acao == "EV_SELECT") {
    ...
    $v_sql = "SELECT * FROM db_adm.t_grupo_noticias WHERE Id = " . $v_id;
...
if ($v_acao == "EV_NOVO") {
    ...
    $v_sql = "INSERT INTO db_adm.t_grupo_noticias (Nome, Descricao)" . "\n" .
...
if ($v_acao == "EV_SALVAR") {
    ...
    $v_sql = "UPDATE db_adm.t_grupo_noticias SET \n" .
...
if($v_acao == 'EV_MOSTRA_GERENCIAMENTO'){
    ...
    $v_sql = "select emp.id, emp.nome, emp.id_grupo_not from db_adm.t_empresas emp order by nome";
    ...
    while ($row = pg_fetch_assoc($result)) {
        $v_empresas[] = array(
            "Id" => $row["id"],
            "nome" => $row["nome"],
            "grupo_noticia" => $row["id_grupo_not"]
        );
    }
    ...
    $v_sql = "select id_usuario from db_adm.t_postagem_noticia where id_grupo = {$v_id}";
...
if($v_acao == 'EV_SALVA_DEF_GRUPO'){
    ...
    $v_sql = "delete from db_adm.t_postagem_noticia where id_grupo = {$v_id_grupo}";
    ...
    if(gettype($v_usuarios) == 'array'){
        $insert_line = 'insert into db_adm.t_postagem_noticia (id_usuario, id_grupo) values ';
    ...
    $v_sql = "update db_adm.t_empresas set id_grupo_not = null where id_grupo_not = {$v_id_grupo}";

    pg_query($conn, $v_sql);

    if(gettype($v_empresas) == 'array'){
        foreach($v_empresas as $empresa){
            $v_sql = "update db_adm.t_empresas set id_grupo_not = {$v_id_grupo} where id = {$empresa}";
