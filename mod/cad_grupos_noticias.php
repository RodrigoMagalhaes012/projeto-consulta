...
if (strpos($_SESSION["vs_array_access"], "T0056") == 0) {
...
<script language="JavaScript">
    ...
    function func_carrega_tab() {
        ...
        $.ajax({
            type: "POST",
            url: "../mod/lib/lib_cad_grupos_noticias.php",
            data: {
                "v_acao": v_acao
            },
            success: function(data) {
                ...
                data.grupos.forEach(element => {
                    options += `<tr style="cursor: pointer;">
                        <td>${element.Id} </td>
                        <td>${element.Nome}</td>
                        <td>${element.Descricao == null ? '-' : element.Descricao}</td>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.empresas.forEach(element => {
                    options += `<option ${element.grupo_noticia == v_id ? 'selected' : ''}  value="${element.Id}">${element.nome}</option>`
                });
